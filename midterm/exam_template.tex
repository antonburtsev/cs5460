...
\question Alice works on implementing a new shell for xv6. She implements a pipe command (e.g., ls | wc) like this: 
...
\part[5] Her implementation always waits for left side to finish, but the shell
that xv6 implements (sh.c in the xv6 source tree) launches the right side right
away. Can you come up with an example when Alice's shell fails, while the xv6's
is still correct? Explain your answer. 
...
\part[5] In xv6 individual processes cannot access each others memory. Explain
