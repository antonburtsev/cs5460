...
\begin{parts}
  ...
  \part[10] Ben puts the code of the test() function inside a new system call trying to see if it works inside the kernel. Will it work (explain your answer)? 
  \vfill
...
\newpage
  \part[15] Ben plans to catch the exception caused by an unmapped page access inside the trap() function. Provide a sketch for the code that implements the exception handling, reads page from disk, and maps it back into the process address space. 
...
\part[5] Explain what the above section of code does?
  ...
  \vfill
\newpage
\part[10] Explain what can go wrong? Quote a concrete example and explain
  the behavior without the above piece of code.
  \vfill
\end{parts}
